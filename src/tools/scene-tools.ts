...
Examples:
  - Use when: "What scenes are open?" or "List all scenes"`,
    ...
    async () => {
      const scenes = await godotRequest<GodotSceneInfo[]>("GET", "/scenes");
      return {
...
Examples:
  - Use when: "Show me the scene tree" or "What nodes are in res://player.tscn?"`,
    ...
    async ({ scene_path, depth }) => {
      const tree = await godotRequest<GodotNodeInfo>("GET", "/scene/tree", { scene_path, depth });
      return {
...
Examples:
  ...
  - Use when: "Get transform of res://enemy.tscn root node"`,
    ...
    async ({ node_path, scene_path }) => {
      const node = await godotRequest<GodotNodeInfo>("GET", "/scene/node", { node_path, scene_path });
      return {
...
Returns:
  Confirmation message.`,
    ...
    async ({ node_path, scene_path }) => {
      await godotRequest<void>("DELETE", "/scene/node", { node_path, scene_path });
      return {
...
Examples:
  ...
  - Use when: "Hide the HUD node"
    ...
    async ({ node_path, properties, scene_path }) => {
      const result = await godotRequest<Record<string, unknown>>("PUT", "/scene/node/properties", {
...
Returns:
  New node path after reparenting.`,
    ...
    async ({ node_path, new_parent_path, scene_path, keep_global_transform }) => {
      const result = await godotRequest<{ new_path: string }>("POST", "/scene/node/reparent", {
...
Examples:
  - Use when: "Add a Goblin enemy to the level at position (300, 100)"
    ...
    async ({ scene_to_instantiate, parent_path, node_name, target_scene_path, properties }) => {
      const result = await godotRequest<GodotNodeInfo>("POST", "/scene/instantiate", {
...
Returns:
  Confirmation with the saved path.`,
    ...
    async ({ scene_path, save_as }) => {
      const result = await godotRequest<{ saved_path: string }>("POST", "/scene/save", { scene_path, save_as });
      return {
