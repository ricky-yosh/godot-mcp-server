...
import { GodotFileInfo, GodotScriptResult } from "../types.js";
...
Returns:
  { path: string, content: string }`,
    ...
    async ({ file_path }) => {
      const file = await godotRequest<GodotFileInfo>("GET", "/filesystem/file", { file_path });
      return {
...
Examples:
  - Use when: "Create a new movement script for the Player"`,
    ...
    async ({ file_path, content }) => {
      await godotRequest<void>("POST", "/filesystem/file", { file_path, content });
      return {
...
Warning: This runs arbitrary code in the editor. Use with care.`,
    ...
    async ({ code, timeout_ms }) => {
      const result = await godotRequest<GodotScriptResult>("POST", "/script/run", { code, timeout_ms });
