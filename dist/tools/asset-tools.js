...
export function registerAssetTools(server) {
    // ── List filesystem ─────────────────────────────────────────────────────
    server.registerTool("godot_list_files", {
...
Returns:
  { total, count, offset, has_more, items: [{ path, type, name }] }`,
        ...
        const result = await godotRequest("GET", "/filesystem/list", { directory, filter_type, recursive, limit, offset });
    ...
    // ── Get resource info ───────────────────────────────────────────────────
    server.registerTool("godot_get_resource", {
...
Returns:
  { path, type, name, metadata: object }`,
        ...
        const info = await godotRequest("GET", "/filesystem/resource", { resource_path });
        return {
            content: [{ type: "text", text: JSON.stringify(info, null, 2) }]
        };
    });
    // ── Assign resource to node property ───────────────────────────────────
    server.registerTool("godot_assign_resource", {
  ...
  - Use when: "Set the player sprite texture to res://assets/player.png"
    -> node_path: "/root/Main/Player/Sprite2D", property: "texture",
       resource_path: "res://assets/player.png"`,
        ...
        await godotRequest("POST", "/filesystem/assign-resource", {
